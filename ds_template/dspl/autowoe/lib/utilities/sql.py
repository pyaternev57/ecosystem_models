...
def preprocess_features(model_data: Dict, table_name: str) -> List[str]:
    ...
    for i, feature in enumerate(model_data['features']):
        ...
        if woe['f_type'] == 'real':
            ...
            string = f"(CASE WHEN {feature} IS NULL OR LOWER(CAST({feature} AS {str_type})) = 'nan' THEN NULL WHEN {feature} NOT IN ({big_cats_str}) THEN '{spec_value}' ELSE CAST({feature} AS {str_type}) END) AS {feature}"
